...
#renaming reads so its paired end
#cat SRR1051419.fastq | sed 's/\s.*$//' | sed 's/\(.*\)\./\1\//' > SRR1051419_renamed.fastq 

####2 Align and generate a BAM file. 
#might want to go ahead and run it through high mem node 
TAG="@RG\tID:$SRR\tSM:$SRR\tLB:$SRR" 
bwa mem -t 10 -R $TAG /scratch/aahowel3/HCMV_slim/patient_data_rerun/ref/${ACC}.fa -p ${SRR}_renamed.fastq > SRR.raw.sam
...
freebayes -f /scratch/aahowel3/HCMV_slim/patient_data_rerun/ref/NC_006273.2.fa --ploidy 1 --min-coverage 15 --haplotype-length 0 --min-alternate-count 1 --min-alternate-fraction 0 --pooled-continuous --report-monomorphic -0 -i -X -u --region NC_006273.2:62500-86000 SRR.raw.sorted.mkdup.bam > 475_sam_dupl_rm_no5.vcf
...
gunzip qual0_475_sam_dupl_rm_no5.vcf.gz
#will complian about tabixing it but will deal
bcftools view --max-alleles 2 qual0_475_sam_dupl_rm_no5.vcf > qual0_475_sam_dupl_rm_no5_nomulti.vcf
#will need to add .output to filename so sum stats script will take it
